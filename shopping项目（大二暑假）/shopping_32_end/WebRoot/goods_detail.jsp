...
<c:if test="${empty good}">
		  <jsp:forward page="goodsDetail.do"></jsp:forward>
</c:if>
<c:if test="${userGoods == null}">
		  <jsp:forward page="showAllComment.do?gid=${good.getId()}"></jsp:forward>
</c:if>
...
<body <%=user==null?"":"onload='showEditor()'"%>>
              ...
              <td height="57" valign="top" background="images/shop_07.gif"><table width="100%" height="53"  border="0" cellpadding="0" cellspacing="0">
                <tr align="center">
                  ...
                  <c:if test="${empty user}">
                    ...
                    Member</a></td>
                  <td width="1%"><img src="images/shop_09.gif" width="3" height="57"></td>
                  <td width="10%"><c:if test="${not empty user}">
                  	<a href="cart_see.jsp">
                  </c:if>
                  <c:if test="${empty user}">
                  	<a href="javascript:showMsg('您还没登录!');">
	...
	<table width="100%"  border="0" cellpadding="0" cellspacing="0">
      ...
      <table width="100%" height="151"  border="0" cellpadding="0" cellspacing="0">
        ...
        <tr>
          ...
          <table id="table" width="88%"  border="0" cellspacing="0" cellpadding="0" id="table">
        ...
        <td colspan="2" background="images/shop_16.gif">
		   <!--搜索框-->
			...
			<tr>
			  ...
			  <td width="15%"><input type="submit" class="pill button" value="搜索"></td>
			</tr>
			</table>
			</form>
			<!--搜索框结束-->
            ...
            <tr valign="top">
              <td height="134" align="center">
                  <table width="96%" height="79"  border="0" cellpadding="0" cellspacing="0" class="noborder">
                    <tr>
                      <td colspan="2" class="font">${good.getGname()} </td>
                    </tr>
                    <tr>
                      <td width="37%" height="95" rowspan="4" align="center"><img class="tip2" src="${good.getPhoto()}" width="150" height="100"></td>
                      <td width="63%" style="text-decoration:line-through;">市场价：￥${good.getMarketPrice()}</td>
                    </tr>
                    <tr>
                      <td style="color:#FF0000">商城价：￥${good.getSalPrice()}(${good.getGdiscount()}) </td>
                    </tr>
                    <tr>
                      <td>
						 <input type="button" class="primary button" value="收藏" onclick="addFavorite(${good.getId() })">
					  </td>
                    </tr>
                    <tr>
                      <td>
                         <c:choose>
                             <c:when test="${good.getNumber() - good.getSalCount()>0}"> <input type="button" class="positive button" value="购买" onclick="addCart(${good.getId() })">&nbsp;&nbsp;</c:when>
                             <c:otherwise>该商品缺货，欢迎选购其他商品</c:otherwise>
                          </c:choose>
					  </td>
                    </tr>
                    </table>
                    <table width="96%" border="0" cellpadding="0" cellspacing="0" class="noborder">
	                    ...
	                    <tr>  
		                      ...
		                      <td>
		                      <c:if test="${empty user}">                      	
		                        <input type="button" class="positive button" value="我要评论" onclick="comment()">
							  </c:if>
							  </td>			
	                    </tr>
                   <c:forEach items="${userGoods}" var="comment">
	                    <tr>

	                    <c:choose>	                   		
	                    	<c:when test="${not empty userGoods}">	
	                    	 <td width="40%"><hr align="center" size="1">                   	
	                    			评论：${comment.getComment()}      	            			   	
	                    	</td>
	                    	
	                    	<td><hr align="center" size="1">
	                    		评论时间：	<fmt:formatDate value="${comment.getCommentTime()}"></fmt:formatDate> 
	                    		&nbsp;&nbsp;&nbsp;
	                    		<c:if test="${empty user}">
	                    		评论者：		登陆后可见  	
	                    		</c:if>
	                    		<c:if test="${not empty user}">
	                    		评论者：		${user.getUsername()}   	
	                    		</c:if>
	                    	</td>                
	                    	</c:when>   
	                    </c:choose>      
            ...
            <tr>
              <td height="38" align="right" background="images/center02.gif">
              	...
              	<form action="">
              			...
              			<input type="hidden" id="gid" name="gid" value="${good.getId()}">
              			<textarea rows="10" cols="60" name="content"></textarea>
              			<center>
              				<input type="button" class="positive button" value="提交"  name="comment" onclick="submitContent();">
              				<input type="reset" class="negative button" value="重置"  name="reset">
              			</center>
